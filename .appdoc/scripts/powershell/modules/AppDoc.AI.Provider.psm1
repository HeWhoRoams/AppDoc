...
# Purpose: Resolve AI execution mode with local-first defaults and invoke explicit AI providers when requested.
    ...
    switch ($normalizedRequest) {
        ...
        "Agent" {
            if ($agentEnabled) {
                ...
                $resolvedMode = "Deterministic"
                $provider = "deterministic"
                $reason = "Agent bridge unavailable."
            }
        }
        default {
            $resolvedMode = "Deterministic"
            $provider = "deterministic"
            $reason = "Auto mode defaults to local deterministic execution. Use explicit Agent or ApiKey mode to run provider-backed AI passes."
        }
    }
...
function Invoke-AppDocOpenAIJsonPass {
    ...
    try {
        $response = Invoke-RestMethod -Method Post -Uri "https://api.openai.com/v1/chat/completions" -TimeoutSec 120 -Headers @{
