...
foreach ($file in $tsFiles) {
    ...
    foreach ($match in $exports) {
        ...
        $records += [ordered]@{
            ...
            metadata = @{ language = "TypeScript" }
            provider = $provider
            confidence = if ($provider -eq "typescript-compiler") { 0.8 } else { 0.55 }
        }
        ...
        if ($match.Groups[1].Value -in @("class", "interface")) {
            ...
            $records += [ordered]@{
                ...
                metadata = @{ modelType = $match.Groups[1].Value; properties = $modelProperties; language = "TypeScript" }
                provider = $provider
                confidence = if ($provider -eq "typescript-compiler") { 0.82 } else { 0.58 }
            }
        }
    }
    ...
    foreach ($match in $decorators) {
        ...
        $records += [ordered]@{
            ...
            metadata = @{ language = "TypeScript" }
            provider = $provider
            confidence = if ($provider -eq "typescript-compiler") { 0.78 } else { 0.5 }
        }
    }
    ...
    foreach ($route in $expressRoutes) {
        ...
        $records += [ordered]@{
            ...
            provider = $provider
            confidence = if ($provider -eq "typescript-compiler") { 0.84 } else { 0.62 }
        }
    }
}
...
$payload = [ordered]@{
    ...
    providerReady = $nodeExists
    records = $records
    note = if ($nodeExists) { "Node.js detected, but TypeScript Compiler API integration is pending. Using regex fallback parser." } else { "Node.js not available; using regex fallback." }
}
