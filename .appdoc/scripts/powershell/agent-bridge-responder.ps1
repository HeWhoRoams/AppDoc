...
.PARAMETER OpenHandoffInCode
    Open generated handoff markdown in VS Code if `code` CLI is available.
...
while ($true) {
    ...
    foreach ($promptFile in $promptFiles) {
        ...
        if (-not $seenPromptPaths.ContainsKey($promptFile.FullName)) {
            ...
            if ($OpenHandoffInCode -and (Get-Command code -ErrorAction SilentlyContinue)) {
                & code $handoffPath | Out-Null
            }
